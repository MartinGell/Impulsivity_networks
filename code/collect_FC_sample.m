...
sample = 'all'    % which sample: all, adults, ...
...
if denoise == 1
    ...
    BV = readtable('/home/mgell/Work/Impulsivity_networks/text_files/brainsize.csv'); % needs a csv with varaibles to denoise*N with subject id as column header
    BV = table2array(BV(1,:));
    subs = readtable('/home/mgell/Work/Impulsivity_networks/text_files/final_subs.csv'); 
    subs.BV = BV';
    subs(subs.age < 19,:) = [];
    subs = subs(1:100,:);
...
sample_subs = table2array(readtable(fullfile('/home/mgell/Work/Impulsivity_networks/Impulsivity_networks/text_files', [sample '.csv']))); % list of subjects to average FC over
...
for file_i = 1:length(sample_subs)
    ...
    sub_i = matlab.lang.makeValidName(sub_i);
    try
    subFC = load(fullfile(inputdir, [sub_i{1} '.mat']));
    catch
...
imagesc(mean_s2sFC);
%imagesc(mean_fc(indsort,indsort));
hold on;
